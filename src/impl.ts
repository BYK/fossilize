...
export default async function (
  ...
  const createBinaryForPlatform = async (platform: string): Promise<void> => {
    ...
    // will be signed — and generate the cache with it. postject then strips
    // that signature when the blob is injected below, and the final signature
    // is (re)applied after inject + hole-punch. Generating with a
    // differently-signed binary makes V8 reject the cache at runtime
    // ("Code cache data rejected"). See #28.
